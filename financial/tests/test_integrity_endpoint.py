...
def test_integrity_endpoint_ok(transactional_db):
    with schema_context('demo'):
        ...
        url = reverse('financial:integrity-check') + f"?building={building.id}"
        resp = client.get(url)
        assert resp.status_code == status.HTTP_200_OK
        data = resp.json()
        assert data.get('status') in ['ok', 'issues_found', 'error']


def test_integrity_endpoint_auto_fix(transactional_db):
    with schema_context('demo'):
        ...
        client = APIClient(); client.force_authenticate(user=user)
        url = reverse('financial:integrity-check') + f"?building={building.id}&auto_fix=true"
        resp = client.get(url)
        assert resp.status_code == status.HTTP_200_OK
