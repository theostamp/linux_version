...
def test_issue_unified_receipt_creates_expense_and_links(transactional_db):
...
def test_cancel_unified_receipt_creates_credit_and_sets_cancelled(transactional_db):
    with schema_context('demo'):
        building = Building.objects.create(name='Test', address='Addr')
...
@pytest.mark.django_db
def test_delete_requires_admin_permissions(transactional_db):
    with schema_context('demo'):
...
@pytest.mark.django_db
def test_financial_system_health_endpoint(transactional_db):
    with schema_context('demo'):
