#!/usr/bin/env python3
"""
Test script για τη νέα λογική κατάστασης βασισμένη στον χρόνο καθυστέρησης
"""
...
def test_new_status_logic():
    """Test της νέας λογικής κατάστασης"""
    
    with schema_context('demo'):
        print("🧪 Testing νέα λογική κατάστασης βασισμένη στον χρόνο καθυστέρησης")
        print("=" * 80)
        ...
        for apartment in apartments:
            ...
            if last_payment_date:
                delay_days = (date.today() - last_payment_date).days
            else:
                # Αν δεν έχει πληρώσει ποτέ, υπολογίζουμε από την πρώτη χρέωση
                first_charge = Transaction.objects.filter(
                    apartment=apartment,
                    type__in=['common_expense_charge', 'expense_payment']
                ).order_by('date').first()
                if first_charge:
                    # Convert datetime to date for comparison
                    first_charge_date = first_charge.date.date() if hasattr(first_charge.date, 'date') else first_charge.date
                    delay_days = (date.today() - first_charge_date).days
            ...
            if delay_days is not None:
                print(f"   Ημέρες καθυστέρησης: {delay_days}")
            if last_payment_date:
