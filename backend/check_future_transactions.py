...
def check_future_transactions():
    """Check for future transactions and potential date discrepancies"""
    
    building_id = 6  # Αλκμάνος 22
    today = date.today()
    
    with schema_context('demo'):
        print("🔍 ΕΡΕΥΝΑ ΜΕΛΛΟΝΤΙΚΩΝ ΣΥΝΑΛΛΑΓΩΝ")
        print("=" * 60)
        ...
        if future_discrepancies:
            print("\n⚠️ ΑΣΥΜΦΩΝΙΕΣ ΜΕ ΜΕΛΛΟΝΤΙΚΕΣ ΗΜΕΡΟΜΗΝΙΕΣ:")
            print("-" * 120)
            print(f"{'Διαμέρισμα':<12} {'Πληρωμή Ημ/νία':<15} {'Συναλλαγή Ημ/νία':<15} {'Διαφορά':<10} {'Ποσό':<12} {'Τύπος'}")
            print("-" * 120)
            
            for disc in future_discrepancies:
