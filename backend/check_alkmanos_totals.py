#!/usr/bin/env python3
"""
Script για έλεγχο αθροισμάτων στο κτίριο Αλκμάνος 22
Ελέγχει γιατί δεν ταιριάζουν τα ποσά στις στήλες του πίνακα κοινοχρήστων
"""
...
def find_alkmanos_building():
    """Βρίσκει το κτίριο Αλκμάνος 22"""
    with schema_context('demo'):
        try:
            ...
            print("❌ Δεν βρέθηκε κτίριο με διεύθυνση που περιέχει 'Αλκμάνος 22'")
            # Ας δοκιμάσουμε να βρούμε κτίρια με 'Αλκμάνος'
            buildings = Building.objects.filter(address__icontains='Αλκμάνος')
            if buildings.exists():
                print(f"📋 Βρέθηκαν κτίρια με 'Αλκμάνος':")
                for b in buildings:
            ...
            print(f"⚠️ Βρέθηκαν {buildings.count()} κτίρια:")
            for b in buildings:
                print(f"   - {b.name}: {b.address}")
            return buildings.first()
    ...
    with schema_context('demo'):
        ...
        try:
            ...
            for apartment_id, share_data in shares.items():
                ...
                # Δαπάνες διαχείρισης (ανά διαμέρισμα)
                management_fee = breakdown.get('management_fee', 0)
                ...
                print(f"     Δαπάνες διαχείρισης: {management_fee}€")
            ...
            # Υπολογισμός δαπανών διαχείρισης
            total_management_fees = sum(
                share_data['breakdown'].get('management_fee', 0) for share_data in shares.values()
            )
            print(f"   Συνολικές δαπάνες διαχείρισης: {total_management_fees}€")
            
            # Έλεγχος αν υπάρχει αμοιβή διαχείρισης στο building
            building = Building.objects.get(id=building_id)
            expected_management_fees = building.management_fee_per_apartment * len(shares) if building.management_fee_per_apartment else 0
            print(f"   Αναμενόμενες δαπάνες διαχείρισης: {expected_management_fees}€")
            
            # Έλεγχος αν ταιριάζουν τα αθροίσματα
            # Οι δαπάνες διαχείρισης είναι ήδη συμπεριλαμβανμένες στις γενικές δαπάνες
...
def main():
    """Κύρια συνάρτηση"""
    print("🔍 ΕΛΕΓΧΟΣ ΑΘΡΟΙΣΜΑΤΩΝ ΚΤΙΡΙΟΥ ΑΛΚΜΑΝΟΣ 22")
    print("=" * 80)
    
    # Βρίσκουμε το κτίριο
    building = find_alkmanos_building()
    if not building:
        return
