#!/usr/bin/env python3
"""
🔍 Τελικός Έλεγχος Payment System - New Concierge (Tenant-Aware)
================================================================
...
Εκτέλεση: docker exec -it linux_version-backend-1 python /app/final_payment_system_verification_tenant.py
"""
...
def check_tenant_data():
    """Έλεγχος δεδομένων tenant"""
    print_section("Έλεγχος Tenant Δεδομένων")
    
    try:
        # Έλεγχος demo tenant
        client = Client.objects.get(schema_name='demo')
        print_info(f"Tenant: {client.name} ({client.schema_name})")
        
        with tenant_context(client):
            # Έλεγχος ότι υπάρχουν κτίρια
            buildings = Building.objects.all()
            print_info(f"Κτίρια στο tenant: {buildings.count()}")
            
            for building in buildings:
                print_info(f"  - {building.name}: {building.address}")
                ...
                print_info(f"    Εισπράξεις: {payments.count()}")
                
                # Έλεγχος transactions
                transactions = Transaction.objects.filter(building=building)
                print_info(f"    Συναλλαγές: {transactions.count()}")
        
        return True
    except Client.DoesNotExist:
        print_error("Demo tenant δεν βρέθηκε")
        return False
    except Exception as e:
        print_error(f"Σφάλμα κατά τον έλεγχο tenant: {e}")
        return False
...
def check_database_consistency():
    ...
    try:
        ...
        with tenant_context(client):
            ...
            payment_transaction_mismatch = 0
            ...
            for payment in payments:
                ...
                if transactions.count() == 0:
                    ...
                    payment_transaction_mismatch += 1
            ...
            if payment_transaction_mismatch == 0:
    ...
    try:
        ...
        with tenant_context(client):
            ...
            if balance_mismatches == 0:
                print_success("✅ Όλοι οι υπολογισμοί υπολοίπων είναι σωστοί")
    ...
    try:
        ...
        with tenant_context(client):
            ...
            for payment in payments:
                ...
                if missing_fields:
                    print_error(f"Payment {payment.id}: Λείπουν πεδία: {missing_fields}")
...
def main():
    """Κύρια συνάρτηση"""
    print_header("ΤΕΛΙΚΟΣ ΕΛΕΓΧΟΣ PAYMENT SYSTEM (TENANT-AWARE)")
    print_info("Έναρξη ελέγχου...")
    
    checks = [
        ("Tenant Data", check_tenant_data),
