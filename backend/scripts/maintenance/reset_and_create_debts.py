...
# -*- coding: utf-8 -*-
"""
Μηδενισμός υπολοίπων και δημιουργία οφειλών για δοκιμή
"""
...
def reset_and_create_debts():
    """Μηδενισμός υπολοίπων και δημιουργία οφειλών"""
    
    with schema_context('demo'):
        print("🔄 ΜΗΔΕΝΙΣΜΟΣ ΥΠΟΛΟΙΠΩΝ ΚΑΙ ΔΗΜΙΟΥΡΓΙΑ ΟΦΕΙΛΩΝ")
        print("=" * 60)
        ...
        # Μηδενισμός όλων των υπολοίπων
        print("🔄 ΜΗΔΕΝΙΣΜΟΣ ΥΠΟΛΟΙΠΩΝ")
        print("-" * 30)
        
        for apt in apartments:
            if apt.current_balance != 0:
                # Δημιουργία συναλλαγής για μηδενισμό
                transaction = Transaction.objects.create(
                    building=building,
                    apartment=apt,
                    amount=-apt.current_balance,  # Αρνητικό για μηδενισμό
                    type='balance_adjustment',
                    description=f"Μηδενισμός υπολοίπου - {apt.number}",
                    date=datetime(2025, 7, 31, 10, 0, 0),
                    balance_before=apt.current_balance,
                    balance_after=Decimal('0.00')
                )
                
                apt.current_balance = Decimal('0.00')
                apt.save()
                print(f"🔄 Μηδενίστηκε υπόλοιπο: Διαμέρισμα {apt.number}")
        ...
        for debt_data in debts_data:
            ...
            if apartment:
                ...
                transaction = Transaction.objects.create(
                    ...
                    balance_after=apartment.current_balance - debt_data['debt_amount']
                )
                
                # Ενημέρωση του υπολοίπου του διαμερίσματος
                apartment.current_balance = apartment.current_balance - debt_data['debt_amount']
                apartment.save()
