#!/usr/bin/env python3
"""
Script για δημιουργία προκαθορισμένων πακέτων υπηρεσιών
"""

import os
import django

# Setup Django
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'new_concierge_backend.settings')
django.setup()

from django_tenants.utils import tenant_context
from tenants.models import Client
from buildings.models import ServicePackage
from decimal import Decimal

def create_service_packages():
    """Δημιουργεί προκαθορισμένα πακέτα υπηρεσιών"""
    
    print("📦 ΔΗΜΙΟΥΡΓΙΑ ΠΑΚΕΤΩΝ ΥΠΗΡΕΣΙΩΝ")
    print("=" * 60)
    
    # Get demo tenant
    try:
        client = Client.objects.get(schema_name='demo')
        print(f"✅ Βρέθηκε tenant: {client.name}")
    except Client.DoesNotExist:
        print("❌ Δεν βρέθηκε demo tenant")
        return
    
    # Check in tenant context
    with tenant_context(client):
        # Delete existing packages
        ServicePackage.objects.all().delete()
        print("🗑️ Διαγράφηκαν υπάρχοντα πακέτα")
        
        # Create predefined packages
        packages_data = [
            {
                'name': 'Βασικό Πακέτο',
                'description': 'Βασικές υπηρεσίες διαχείρισης πολυκατοικίας',
                'fee_per_apartment': Decimal('5.00'),
                'services_included': [
                    'Διαχείριση κοινόχρηστων',
                    'Εκκαθάριση λογαριασμών',
                    'Συντονισμός επισκευών',
                    'Επικοινωνία με ενοικιαστές'
                ]
            },
            {
                'name': 'Πρότυπο Πακέτο',
                'description': 'Πρότυπες υπηρεσίες διαχείρισης με επιπλέον χαρακτηριστικά',
                'fee_per_apartment': Decimal('10.00'),
                'services_included': [
                    'Διαχείριση κοινόχρηστων',
                    'Εκκαθάριση λογαριασμών',
                    'Συντονισμός επισκευών',
                    'Επικοινωνία με ενοικιαστές',
                    'Διαχείριση αποθεματικού',
                    'Ετήσιες εκθέσεις',
                    'Συντονισμός καθαρισμού'
                ]
            },
            {
                'name': 'Προηγμένο Πακέτο',
                'description': 'Προηγμένες υπηρεσίες διαχείρισης με πλήρη υποστήριξη',
                'fee_per_apartment': Decimal('15.00'),
                'services_included': [
                    'Διαχείριση κοινόχρηστων',
                    'Εκκαθάριση λογαριασμών',
                    'Συντονισμός επισκευών',
                    'Επικοινωνία με ενοικιαστές',
                    'Διαχείριση αποθεματικού',
                    'Ετήσιες εκθέσεις',
                    'Συντονισμός καθαρισμού',
                    'Διαχείριση ασφαλιστικών',
                    'Συντονισμός ασφάλειας',
                    '24/7 υποστήριξη',
                    'Ψηφιακή πλατφόρμα'
                ]
            },
            {
                'name': 'Premium Πακέτο',
                'description': 'Premium υπηρεσίες διαχείρισης με εξατομικευμένη φροντίδα',
                'fee_per_apartment': Decimal('25.00'),
                'services_included': [
                    'Διαχείριση κοινόχρηστων',
                    'Εκκαθάριση λογαριασμών',
                    'Συντονισμός επισκευών',
                    'Επικοινωνία με ενοικιαστές',
                    'Διαχείριση αποθεματικού',
                    'Ετήσιες εκθέσεις',
                    'Συντονισμός καθαρισμού',
                    'Διαχείριση ασφαλιστικών',
                    'Συντονισμός ασφάλειας',
                    '24/7 υποστήριξη',
                    'Ψηφιακή πλατφόρμα',
                    'Εξατομικευμένη φροντίδα',
                    'Συντονισμός κήπου',
                    'Διαχείριση parking',
                    'Συντονισμός events'
                ]
            }
        ]
        
        created_packages = []
        for package_data in packages_data:
            package = ServicePackage.objects.create(**package_data)
            created_packages.append(package)
            print(f"✅ Δημιουργήθηκε: {package.name} - {package.fee_per_apartment}€/διαμέρισμα")
        
        print(f"\n🎉 Δημιουργήθηκαν {len(created_packages)} πακέτα υπηρεσιών!")
        
        # Show summary
        print("\n📊 ΣΥΝΟΨΗ ΠΑΚΕΤΩΝ:")
        for package in created_packages:
            print(f"   - {package.name}: {package.fee_per_apartment}€/διαμέρισμα")
            print(f"     Υπηρεσίες: {package.get_services_list()}")
            print()

if __name__ == "__main__":
    create_service_packages()
