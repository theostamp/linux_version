...
### ✅ Required Actions Before Commands
...
- **User Model**: Use `from users.models import CustomUser` (not django.contrib.auth.models.User)
- **User Creation**: CustomUser requires email field: `User.objects.create_user(email='test@example.com', password='testpass')`
...
- **Greek Characters**: Always consider Greek alphabet in apartment numbers (Α1, Β2, Γ3, etc.)
- **Character Encoding**: Use UTF-8 for all Greek text handling
- **Apartment Numbers**: May contain Greek letters (Α, Β, Γ, Δ) or Latin numbers (1, 2, 3)
- **Database Queries**: When searching for apartments, consider both Greek and Latin characters
- **Example**: `'Α1'` (Greek Alpha) vs `'A1'` (Latin A) are different

### 🔍 Common Debugging Patterns
- **Apartment Not Found**: Check if using correct character encoding (Greek vs Latin)
- **Database Connection**: Always use Docker containers, never local connections
- **Virtual Environment**: Always activate .venv before Python operations
- **Tenant Context**: All database operations must be within `schema_context('demo')`

### 🧪 API Testing Requirements
- **Request Factory**: Use `from django.test import RequestFactory`
- **Query Params**: Add `request.query_params = request.GET` for ViewSet testing
- **User Authentication**: Use CustomUser model, not django.contrib.auth.models.User
- **Response Status**: Always check `response.status_code` before accessing data
- **Data Access**: Use `response.data` for ViewSet responses
...
- Common expenses (κοινόχρηστα) with proper breakdowns
...
- **previous_balance**: Previous month's ending balance (use for "Παλαιότερες Οφειλές")
- **expense_share**: Current month's expense allocation (use for "Ποσό Κοινόχρηστων")
- **net_obligation**: Total obligations minus payments (includes current month)
- **current_balance**: Real-time balance from transaction history
- **total_obligations**: Historical + current obligations
- **total_payments**: Historical + current payments

### Common Financial Issues
- **Wrong Previous Obligations**: Use `previous_balance` NOT `net_obligation` for "Παλαιότερες Οφειλές"
- **Wrong Common Expenses**: Use `expense_share` NOT `net_obligation` for "Ποσό Κοινόχρηστων"
- **Balance Mismatch**: Check transaction history vs calculated balance
- **Greek Currency**: Always use € symbol and proper Greek formatting
