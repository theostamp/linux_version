...
import type { KioskAnnouncement } from '@/hooks/useKioskData';
...
export default function AnnouncementsWidget({ widget, data, isLoading, error, settings }: BaseWidgetProps) {
  ...
  const announcements = (data?.announcements || []) as KioskAnnouncement[];
        ...
        {announcements.length > maxItems && (
          <div className="text-center py-2">
            <p className="text-xs text-blue-300">
              +{announcements.length - maxItems} περισσότερες ανακοινώσεις
            </p>
          </div>
        )}
      </div>
