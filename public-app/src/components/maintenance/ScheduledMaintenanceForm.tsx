  ...
  useEffect(() => {
    const loadContractors = async () => {
      try {
        ...
        const resp = await api.get('/maintenance/contractors/', { page_size: 1000 });
  ...
  useEffect(() => {
    const loadExisting = async () => {
      ...
      try {
        ...
        try {
          // The api.get returns data directly
          const resp = await api.get('/maintenance/payment-schedules/', { scheduled_maintenance: data.id });
          const schedule = Array.isArray(resp) ? (resp[0] || null) : (resp?.results?.[0] || null);
          if (schedule) {
  ...
  useEffect(() => {
    ...
    if (initialData) {
      const resetData = {
        ...
        scheduled_date: initialData.scheduled_date ? formatForDateInput(initialData.scheduled_date) : undefined,
        priority: initialData.priority ?? 'medium',
        status_ui: (initialData.status === 'completed' ? 'executed' : 'pending') as 'pending' | 'executed',
        estimated_duration: 1,
    ...
    try {
      ...
      for (let i = 1; i <= installmentCount; i++) {
        ...
        const installmentExpense = await createExpense({
          ...
          category: category,
          distribution_type: 'by_participation_mills',
          notes: `Δόση ${i} από ${installmentCount} για συντήρηση. Ποσό δόσης: ${installmentAmount.toFixed(2)}€. Προγραμματισμένο έργο #${scheduledMaintenanceId}`,
        } as any);
      ...
      if (pc.payment_type === 'advance_installments' && pc.total_amount && pc.installment_count) {
        ...
        const relatedExpenses = existingExpenses.filter(expense => {
          ...
          return (
            ...
            (expenseTitle.includes(`συντήρηση: ${values.title.toLowerCase()}`))
          );
        });
