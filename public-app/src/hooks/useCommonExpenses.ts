...
export const useCommonExpenses = () => {
  ...
  const getCommonExpensePeriods = useCallback(async (buildingId?: number): Promise<CommonExpensePeriod[]> => {
    ...
    try {
      ...
      const response = await api.get<{ results?: CommonExpensePeriod[] } | CommonExpensePeriod[]>(`/financial/common-expense-periods/?${searchParams}`);

      const result = Array.isArray(response) ? response : (response as { results?: CommonExpensePeriod[] }).results;
