...
function MarketplaceContent() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 space-y-10">
      ...
      {/* Hero */}
      <div className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-800 p-10 md:p-14 shadow-2xl shadow-indigo-500/10">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="md:col-span-2 relative group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                <Search className="w-5 h-5 text-indigo-200 group-focus-within:text-indigo-500 transition-colors" />
              </div>
              <Input
                placeholder="Αναζήτηση (π.χ. υδραυλικός, εταιρεία)..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="pl-12 h-12 bg-white/95 backdrop-blur-md border-0 rounded-2xl text-indigo-900 placeholder:text-indigo-300 font-medium focus-visible:ring-4 focus-visible:ring-white/20 shadow-lg"
              />
            </div>
            <select
              value={serviceType}
              onChange={(e) => setServiceType(e.target.value)}
              className="h-12 rounded-2xl bg-white/95 text-indigo-900 font-medium px-4 border-0 shadow-lg focus:outline-none focus:ring-4 focus:ring-white/20"
            >
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm text-indigo-100">
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (!raw) return setMaxDistanceKm(null);
                const parsed = Number(raw);
                setMaxDistanceKm(Number.isFinite(parsed) ? parsed : null);
              }}
              placeholder="π.χ. 30"
              className="w-32 h-10 bg-white/95 text-indigo-900 rounded-xl border-0"
            />
            {maxDistanceKm ? (
              <Button
                variant="ghost"
