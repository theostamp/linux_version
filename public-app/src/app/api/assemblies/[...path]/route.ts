...
export function middleware(request: NextRequest) {
  console.log("[assemblies proxy:path] incoming", {
    ...
    resolvePath: async (_request, context) => {
      ...
      const pathSegments = Array.isArray(segments)
        ...
        : segments
          ? [segments]
          : [];
      return ["assemblies", ...pathSegments].join("/");
    },
...
const { GET, POST, PUT, PATCH, DELETE, OPTIONS } = exportHandlers(handlers, methods, "assemblies");
